...
export default function NavisWay() {
  ...
  return (
    <div ref={scope} className="bg-deepBlue py-10">
      <div className="max-w-6xl mx-auto py-10 px-4 md:px-8 space-y-10 text-center">
        ...
        <div className="mt-10 grid grid-cols-1 gap-4 md:hidden">
          {navisWay.map((item) => (
            ...
            >
              <h2 className="font-heading text-deepOrange font-bold text-[34px]">
          ...
          {navisWay.map((item) => (
            <div key={item.id} className="contents">
              <div data-reveal className="col-span-1 lg:col-span-2">
                <h2 className="font-heading text-deepOrange font-bold text-[48px]">
                  <span data-count={item.tag}>{item.tag}</span>
                </h2>
              </div>
              <div data-reveal className="col-span-4 lg:col-span-3">
                <h2 className="font-heading text-mainWhite font-medium text-[32px] text-start">
                  {item.title}
                </h2>
              </div>
